...
public class FeatureExtractor implements Serializable {
	...
	int fValLen;
	// The dictionary of all feature values.
	 ...
	 * 
	 * @param docs
	 *            : documents to extract feature for.
	 * @param deps
	 *            : Dependency file manually created.
	 * @param b
	 *            : boolean value to indicate if it's training or testing.
	 * @return feature array.
	 * @throws Exception
	 */
	public ArrayList<ArrayList<svm_node>> extract(ArrayList<Document> docs,
			HashMap<String, FileReadDependency> deps, boolean b)
			throws Exception {
		...
		for (String featureHead : featureHeads) {
			...
			if (featureHead.equals("fileparse")) {
				feature = extractFileReadParseFeature(docs, deps, b);
			}
	...
	private ArrayList<ArrayList<svm_node>> extractFileReadParseFeature(
			ArrayList<Document> docs, HashMap<String, FileReadDependency> deps,
			boolean isTraining) throws Exception {
		...
		if (isTraining)
			for (Document doc : docs) {
				...
				String depword = toks.length > 1 ? toks[toks.length - 1]
						: toks[0];
